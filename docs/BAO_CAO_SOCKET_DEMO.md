...
## Xây dựng Web Client - Web Server bằng C++ và Winsock2
...
**Thành viên và mã số sinh viên:** [CẦN BỔ SUNG]  
**Thời gian thực hiện:** Tháng 06 năm 2026

\newpage

# BIÊN BẢN KIỂM TRA DỰ ÁN TRƯỚC KHI VIẾT BÁO CÁO

## 1. Phạm vi kiểm tra

Báo cáo được xây dựng sau khi kiểm tra mã nguồn trong `SocketDemo`, tài liệu `Socket.pdf`, các slide HTTP/Web Server và các chương trình minh họa trong thư mục `Ref`. Việc đánh giá dựa trên nội dung mã nguồn và kết quả chạy thực tế, không dựa riêng vào kế hoạch nâng cấp hoặc mô tả trong README.

Ngày kiểm tra gần nhất là **29/06/2026**. Dự án được build bằng CMake 4.2.3 và MinGW g++ 15.2.0. Lệnh `cmake --build build` hoàn tất với trạng thái không có tệp nào cần biên dịch lại. Bộ `tests/smoke_test.ps1` hoàn thành **34/34 phép kiểm tra**.

## 2. Chức năng đã xác nhận

| Nhóm | Chức năng đã xác nhận | Bằng chứng chính |
|---|---|---|
| Chương trình | Có hai tệp chạy độc lập `socket_server.exe` và `socket_client.exe` | `CMakeLists.txt`, dòng 28-32 |
| Winsock | Khởi tạo Winsock 2.2 bằng `WSAStartup`, giải phóng bằng `WSACleanup` | `src/common/socket_support.cpp`, dòng 10-25 |
| TCP Server | Thực hiện `socket`, `bind`, `listen`, `accept` | `src/server/main.cpp`, dòng 258-295 |
| TCP Client | Phân giải host, tạo socket và gọi `connect` | `src/client/main.cpp`, dòng 26-56 |
| HTTP Client | Tự tạo request `GET ... HTTP/1.1` và gửi qua TCP | `src/client/main.cpp`, dòng 98-109 |
| HTTP Server | Phân tích request line, header và yêu cầu `Host` | `src/common/http.cpp`, dòng 23-71 |
| File tĩnh | Ánh xạ URL thành tệp trong document root và trả nội dung | `src/server/main.cpp`, dòng 193-221 |
| Mã trạng thái | Hỗ trợ `200`, `400`, `404`, `405` | `src/server/main.cpp`, dòng 165-221 |
| Header | Trả `Content-Length`, `Content-Type`, `Connection: close` | `src/common/http.cpp`, dòng 74-92 |
| An toàn đường dẫn | Từ chối `..`, dấu gạch chéo ngược, dấu hai chấm và target bất thường | `src/server/main.cpp`, dòng 111-125 và 193-202 |
| Giới hạn dữ liệu | Giới hạn request header 16 KB, response Client 10 MB | `src/common/http.h`, dòng 10; `src/client/main.cpp`, dòng 12 và 59-75 |
| Đa client | Mỗi kết nối được xử lý bằng một detached thread | `src/server/main.cpp`, dòng 301-329 |
| Logging | Log đồng bộ ra console và `server.log` bằng mutex | `src/server/main.cpp`, dòng 23-52 |
| LAN | Server cho phép cấu hình địa chỉ bind, gồm `0.0.0.0` | `src/server/main.cpp`, dòng 238-271 |
| Kiểm thử | 34 smoke checks đạt ngày 29/06/2026 | `tests/smoke_test.ps1`, dòng 125-179 và kết quả chạy thực tế |

## 3. Chức năng chưa có hoặc chưa được xác nhận

- Chưa có thread pool, hàng đợi hữu hạn và backpressure. Mô hình hiện tại tạo một thread cho mỗi client và không giới hạn số thread.
- Chưa có `SO_RCVTIMEO`, `SO_SNDTIMEO`, mã `408 Request Timeout` hoặc `503 Service Unavailable`.
- Chưa có graceful shutdown; Server dừng bằng `Ctrl+C`, các thread đang detach không được join có kiểm soát.
- Chưa hỗ trợ `HEAD`, `POST`, upload, download theo tên tệp đích, keep-alive, HTTPS hoặc xác thực.
- Chưa có giao diện MFC và chưa sử dụng `WSAAsyncSelect`.
- Chưa có unit test tách riêng cho HTTP parser, socket RAII hoặc mô hình đồng thời.
- Chưa có kết quả load test 500 request hoặc số liệu throughput, average, p95. Báo cáo không tạo số liệu giả.
- Chưa xác nhận bằng máy thứ hai trong LAN ở lần kiểm tra này. Mã nguồn và hướng dẫn hỗ trợ LAN, nhưng đây vẫn là ca kiểm thử thủ công cần nhóm thực hiện.
- Chưa có module UDP. Đây không phải lỗi đối với nhánh Web Server TCP, nhưng là phần có thể bổ sung nếu muốn bao phủ cả hai mô hình trong `Socket.pdf`.

## 4. Thông tin cần người dùng bổ sung

- Tên trường, khoa, môn học và giảng viên hướng dẫn.
- Tên nhóm, danh sách thành viên, mã số sinh viên và phân công thực tế.
- Ảnh chụp màn hình demo và kiểm thử LAN trên hai máy.
- Quy định định dạng báo cáo của giảng viên nếu khác với bố cục hiện tại.
- Kết quả đo tải nếu nhóm thực hiện load test ở giai đoạn sau.

## 5. Dàn ý và ngân sách trang dự kiến

| Phần | Số trang dự kiến |
|---|---:|
| Bìa, lời mở đầu, mục lục, biên bản kiểm tra | 4-5 |
| Chương 1. Tổng quan đề tài | 3-4 |
| Chương 2. Cơ sở lý thuyết | 8-10 |
| Chương 3. Phân tích yêu cầu | 3-4 |
| Chương 4. Phân tích và thiết kế | 5-6 |
| Chương 5. Cài đặt | 5-6 |
| Chương 6. Kiểm thử và đánh giá | 4-5 |
| Chương 7. Đối chiếu tài liệu Ref | 3-4 |
| Chương 8-9, kết luận, tài liệu tham khảo, phụ lục | 5-7 |
| **Tổng dự kiến** | **40-51** |
...
| Thành viên | MSSV | Công việc | Tỷ lệ | Ghi chú |
|---|---|---|---:|---|
| [CẦN BỔ SUNG] | [CẦN BỔ SUNG] | Phân tích và Server | [CẦN BỔ SUNG] |  |
| [CẦN BỔ SUNG] | [CẦN BỔ SUNG] | Client và HTTP | [CẦN BỔ SUNG] |  |
| [CẦN BỔ SUNG] | [CẦN BỔ SUNG] | Kiểm thử và tài liệu | [CẦN BỔ SUNG] |  |

# PHỤ LỤC E. DANH SÁCH HẠNG MỤC BÀN GIAO

- [x] Mã nguồn C++17 của Client và Server.
- [x] CMake build configuration.
- [x] Website mẫu.
- [x] Smoke test 34 checks.
- [x] README hướng dẫn localhost/LAN.
- [x] Báo cáo Markdown và DOCX.
- [ ] Thông tin trường, môn, giảng viên và thành viên.
- [ ] Ảnh chụp demo LAN hai máy.
- [ ] Kết quả load test thực tế.
- [ ] Xác nhận yêu cầu bắt buộc về MFC/WSAAsyncSelect từ giảng viên.
